//---------------------------------------------------------------------------------------
//  FILE:   X2DownloadableContentInfo_ItsJustAScratch_WOTC.uc                                    
//           
...
class X2DownloadableContentInfo_ItsJustAScratch_WOTC extends X2DownloadableContentInfo config(Game);

var config array<name> IncludeCharacters;
var config array<name> ExcludeCharacters;

var config bool bForEveryone;
...
static event OnPostTemplatesCreated()
{
	...
	local name NameIterator;

	Mgr = class'X2CharacterTemplateManager'.static.GetCharacterTemplateManager();

	if (default.bForEveryone)
	{
		Mgr.GetTemplateNames(CharacterNames);
		foreach default.ExcludeCharacters(NameIterator)
		{
			CharacterNames.RemoveItem(NameIterator);
		}
	}
	else
	{
		CharacterNames = default.IncludeCharacters;
	}

	foreach CharacterNames(NameIterator)
	{
		Mgr.FindDataTemplateAllDifficulties(NameIterator, Templates);
		`log("Just a Scratch for" @ NameIterator, Templates.Length > 0);
		foreach Templates(CharTemplate)
		{
			X2CharacterTemplate(CharTemplate).bIgnoreEndTacticalHealthMod = true;
			X2CharacterTemplate(CharTemplate).Abilities.AddItem('JustAScratch_UniversalAbility');
		}
	}
